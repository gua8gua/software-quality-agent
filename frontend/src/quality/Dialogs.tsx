...
export function NewProject({ tenant, close, saved }: { tenant: string; close: () => void; saved: (project: Project) => void }) {
  ...
  return <Modal title="新建软件项目" close={() => !busy && close()}><form onSubmit={submit}>
    ...
    {error && <Alert>{error}</Alert>}<footer><button type="button" disabled={busy} onClick={close}>取消</button><button className="qm-primary" disabled={busy}>{busy ? "正在创建…" : "创建项目"}</button></footer>
  </form></Modal>;
}
...
export function UploadDialog({ tenant, project, dataset, capabilities, close, saved }: { tenant: string; project: Project; dataset?: Dataset; capabilities: Capabilities; close: () => void; saved: (dataset: Dataset) => void }) {
  ...
  return <Modal title="上传项目资料" close={() => !busy && close()} wide><form onSubmit={submit}>
    ...
    <p className="qm-muted">{dataset ? `以 ${dataset.version} 的资料为基础创建新快照，保留原版本。` : "创建项目的第一份资料快照。"} 文件类型由你指定。</p>
    ...
    {error && <Alert>{error}</Alert>}<footer><span>{rows.length} 个文件</span><button type="button" disabled={busy} onClick={close}>取消</button><button className="qm-primary" disabled={busy || !rows.length}>{busy ? "提取正文并保存…" : "上传并保存"}</button></footer>
  </form></Modal>;
}
...
export function AnalysisDialog({ tenant, project, dataset, artifacts, kinds, close, created }: { tenant: string; project: Project; dataset: Dataset; artifacts: Artifact[]; kinds: Map<string, string>; close: () => void; created: (run: Run) => void }) {
  ...
  return <Modal title="重新进行 TLR 检测" close={() => !busy && close()} wide><form onSubmit={submit}>
    <p><strong>{project.name}</strong> / {dataset.version}</p><p className="qm-muted">新建独立运行，保留已有结果。选择有方向的两组资料，例如需求 → 代码。</p>
    <div className="qm-toolbar"><button type="button" onClick={() => preset(false)}>需求 → 代码</button><button type="button" onClick={() => preset(true)}>需求 → 测试</button><button type="button" onClick={() => { setSources([]); setTargets([]); }}>清空选择</button></div>
    <div className="qm-selection"><table><thead><tr><th>制品</th><th>类型</th><th>源资料</th><th>目标资料</th></tr></thead><tbody>{artifacts.map(a => <tr key={a.id}><td>{a.external_id}</td><td>{kinds.get(a.kind) || a.kind}</td><td><input type="checkbox" aria-label={`${a.external_id} 作为源`} checked={sources.includes(a.external_id)} onChange={() => toggle(a.external_id, true)} /></td><td><input type="checkbox" aria-label={`${a.external_id} 作为目标`} checked={targets.includes(a.external_id)} onChange={() => toggle(a.external_id, false)} /></td></tr>)}</tbody></table></div>
    ...
    {error && <Alert>{error}</Alert>}<footer><span>{sources.length} 个源 · {targets.length} 个目标</span><button type="button" disabled={busy} onClick={close}>取消</button><button className="qm-primary" disabled={busy || !sources.length || !targets.length}>{busy ? "创建运行…" : "开始检测"}</button></footer>
  </form></Modal>;
}
