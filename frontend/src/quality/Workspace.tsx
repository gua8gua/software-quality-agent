...
function ProjectPage({ tenant, projectId, capabilities, kinds, begin, refresh }: { refresh: number; tenant: string; projectId: string; capabilities: Capabilities | null; kinds: Map<string, string>; begin: (run: Run | Run[]) => void }) {
  ...
  const [tab, setTab] = useState("data"), [search, setSearch] = useState(""), [kind, setKind] = useState("all"), [page, setPage] = useState(0), [runPage, setRunPage] = useState(0);
