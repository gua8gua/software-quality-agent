...
export function StructureTree({datasetId,tenant,project}:{artifacts:Artifact[];datasetId:string;tenant:string;project:string}){
  ...
  return <div className="qm-inset"><p className="muted-text">仅展示所选层的原始层级：优先使用原数据的树路径和全部父节点，其次使用显式导入层级。保留原始容器和多处出现，不使用 TLR 匹配或模型生成关系。</p>
...
export function StructureDetail({artifact,tenant,project}:{artifact:Artifact;tenant:string;project:string}){
  ...
  return <section className="surface qm-inset"><h2>原始结构关系</h2>{error&&<Alert>{error}</Alert>}<p>原始类型：{s.original_type||artifact.kind} · 正文状态：{s.content_status==="reference_only"?"仅有引用，不能当作源码正文":"有正文"}</p><p>原始标识：{s.original_id||artifact.external_id}</p><h3>父节点</h3><ul>{(s.parent_ids||[]).map(id=><li key={id}>{link(id)}</li>)}</ul><h3>数据集原有关系</h3><ul>{(s.relations||[]).map((r,i)=><li key={i}>{r.relation} / {r.source_relation_type} → {link(r.target_id)}</li>)}</ul><details><summary>完整结构元数据与来源</summary><pre className="qm-document">{JSON.stringify(s,null,2)}</pre></details></section>
}
