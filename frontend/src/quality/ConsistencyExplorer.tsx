...
function UnitTable({ title, rows, model, labels, base }: { title: string; rows: Unit[]; model: ConsistencyModel; labels: Record<Outcome, string>; base: string }) {
  ...
  return <section className="qm-panel qm-unit-list"><div className="qm-inset"><h2>{title} <small>{rows.length} 个单元</small></h2><input aria-label={`搜索${title}`} placeholder="搜索名称或标识…" value={search} onChange={e => { setSearch(e.target.value); setPage(0); }} /></div>
    {!filtered.length ? <Empty title={rows.length ? "没有匹配的单元" : "未保存此层结构"} /> : <><div className="qm-table-wrap"><table><thead><tr><th>单元 / 类型</th><th>所属工件或父单元</th><th>判定情况</th></tr></thead><tbody>{filtered.slice(current * 15, (current + 1) * 15).map(u => <tr key={u.key}>
...
function UnitDetail({ unit, model, report, base, tenant, project, labels }: { unit: Unit; model: ConsistencyModel; report: Report; base: string; tenant: string; project: string; labels: Record<Outcome, string> }) {
  ...
  return <><a className="qm-back" href={`${base}/${unit.type === "artifact" ? "original" : "structure"}`}>返回{unit.type === "artifact" ? "原始工件" : "构建结构"}</a>
    ...
    <section className="qm-panel qm-inset"><h3>单元关系与判定</h3><dl className="qm-unit-meta"><dt>所属原始工件</dt><dd>{unit.artifact ? <a href={unitHref(base, originalKey(unit.role, unit.artifact))}>{model.units.get(originalKey(unit.role, unit.artifact))?.title || unit.artifact}</a> : "跨工件功能分类"}</dd>
    ...
    <section className="qm-panel qm-inset"><h3>{unit.artifact ? "单元原文" : "聚类说明"}</h3>{error ? <Alert>{error}</Alert> : body !== undefined ? <pre className="qm-unit-body">{body}</pre> : artifact ? <Loading /> : <p>历史记录未保存可读取的正文。</p>}</section>
    ...
    <section className="qm-panel qm-inset"><h3>{unit.role === "source" ? "下层单元对各项任务的完成情况" : "此单元承担的上层任务"}</h3><p className="qm-muted">展示本单元及其子单元的已有判定。每条内容记录的多段引文共同构成证据；局部“实现”不等于整条上层需求已完成。</p>
      {!judged.length ? <Empty title="尚无一致性判定记录"><p>{matched.length ? "已有匹配关联，尚不能据此认定任务完成。" : "尚未获得此范围的判定，不能据此认定功能缺失。"}</p></Empty> : <><div className="qm-table-wrap"><table><thead><tr><th>上层任务 / 单元</th><th>下层单元</th><th>完成情况与依据</th></tr></thead><tbody>{judged.slice(current * 15, (current + 1) * 15).map(r => <tr key={r.key}>
    ...
    <p className="qm-muted">当前范围包含 {keys.size} 个单元。</p>
  </>;
}
...
export function ConsistencyExplorer({ report, tenant, project, view, unitId }: { report: Report; tenant: string; project: string; view: string; unitId?: string }) {
  ...
  return <div className="qm-consistency-explorer"><nav className="qm-tabs" aria-label="一致性分析子页面"><a className={!structured && view !== "units" ? "active" : ""} aria-current={!structured && view !== "units" ? "page" : undefined} href={`${base}/original`}>原始工件</a><a className={structured ? "active" : ""} aria-current={structured ? "page" : undefined} href={`${base}/structure`}>构建结构</a></nav>
    {error ? <Alert>{error}</Alert> : loading ? <Loading /> : view === "units" ? selected ? <UnitDetail key={selected.key} unit={selected} model={model} report={report} base={base} tenant={tenant} project={project} labels={labels} /> : <Empty title="单元不存在或不属于此次分析" /> : <>
      <p className="qm-muted">{structured ? "功能聚类、拆分单元、代码语法结构与匹配片段均来自此次分析保存的数据。点击单元查看父子关系及判定。" : "上层为本次分析的源工件，下层为目标工件。点击工件查看正文、拆分结构及双向任务对应。"}</p>
      {structured && !report.snapshot.requirement_structure && <p className="qm-muted">历史分析的完整拆分树优先读取其辅助 TLR 保存的结构；未保存的结构不会重新推断。</p>}
