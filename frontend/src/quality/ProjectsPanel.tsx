...
import { all, date, errorText, get, getArtifactContent, post, runDisplayStatus, statusLabel, url, type Artifact, type Capabilities, type Dataset, type Project, type Run, type Visualization } from "./api";
...
export default function ProjectsPanel({ route, tenant, tenantDraft, onTenantDraft, onTenantChange }: {
  route: string[]; tenant: string; tenantDraft: string;
  onTenantDraft: (value: string) => void; onTenantChange: () => void;
}) {
  ...
  const begin = useCallback((value: Run | Run[]) => {
    ...
    if(batchId&&runs.every(run=>run.config.plan_id===batchId)){
      ...
      navigate(batchPath(runs[0].project_id, batchKey(runs[0])));setActionError("");
  ...
  function applyTenant(event: FormEvent) { event.preventDefault(); if (tenantDraft.trim()) { onTenantChange(); setActionError(""); navigate("/projects"); } }
  return <><header className="panel-header quality-context"><div className="qm-breadcrumb"><a href={isModels ? "#/models" : "#/projects"}>{isModels ? "模型配置" : "项目资料库"}</a>{project && <><ChevronRight size={14} /><a href={`#${projectPath(project)}`}>{project}</a></>}{detail && <><ChevronRight size={14} /><span>{detailLabel[detail] || "资料详情"}</span></>}</div><form className="inline-controls" onSubmit={applyTenant}><label className="field compact" htmlFor="workspace">工作空间<input id="workspace" value={tenantDraft} onChange={e => onTenantDraft(e.target.value)} maxLength={128} required /></label><button className="ghost-button" disabled={!tenantDraft.trim()}>切换</button></form></header>
      <div className="quality-content">{actionError && <Alert>{actionError}</Alert>}
...
function Projects({ tenant }: { tenant: string }) {
  ...
  return <><div className="qm-heading"><div><div className="eyebrow">PROJECT LIBRARY</div><h1>所有项目</h1><p>从项目出发，查看软件资料、版本和追踪检测结果。</p></div><button className="primary-button" onClick={() => setNewProject(true)}><Plus size={18} />新建项目</button></div>
...
function ProjectPage({ tenant, projectId, capabilities, kinds, begin, refresh, initialTab = "data" }: { initialTab?: string; refresh: number; tenant: string; projectId: string; capabilities: Capabilities | null; kinds: Map<string, string>; begin: (run: Run | Run[]) => void }) {
  ...
  return <><a className="qm-back" href="#/projects"><ArrowLeft size={15} />所有项目</a><div className="qm-heading"><div><div className="eyebrow">PROJECT / {project.id}</div><h1>{project.name}</h1><p>{project.description || "查看项目资料与历史分析记录"}</p></div><div className="inline-controls"><button disabled={!datasets.length} onClick={() => setTab("consistency")}><ShieldCheck size={17} />一致性分析</button><button disabled={!capabilities} onClick={() => setUpload(true)}><Upload size={17} />上传资料</button><button className="primary-button" disabled={!dataset || artifacts.length < 2 || inventoryLoading || !!inventoryError} onClick={() => setAnalysis(true)}><GitBranch size={17} />重新进行 TLR 检测</button></div></div>
...
function LegacyArtifactPage({ tenant, project, id, kinds }: { tenant: string; project: string; id: string; kinds: Map<string, string> }) {
  ...
  return <><a className="qm-back" href={`#${projectPath(project)}`}><ArrowLeft size={15} />返回项目</a><div className="qm-heading"><div><div className="eyebrow">ARTIFACT DETAIL</div><h1>{data.external_id}</h1><p>{data.locator}</p></div><a className="qm-button" href={url(`/artifacts/${id}/download`, tenant, project)}><Download size={17} />{data.original_file_id ? "下载原始文件" : "下载正文"}</a></div><div className="qm-detail-grid"><section className="surface"><div className="panel-header"><h2>{raw ? "结构化记录" : "完整正文"}</h2><button onClick={() => setRaw(v => !v)}>{raw ? "查看正文" : "查看 JSON"}</button></div>{raw ? <pre className="qm-document" tabIndex={0}>{JSON.stringify(data, null, 2)}</pre> : typeof data.content === "string" && data.content.length > 0 ? <pre className="qm-document" tabIndex={0}>{data.content}</pre> : <div className="qm-empty" role="status">{data.structure?.content_status === "reference_only" ? "此记录仅包含文件引用，尚未导入文件正文。" : "当前记录没有可显示的正文。"}{data.original_file_id && <p><a href={url(`/artifacts/${id}/download`, tenant, project)}>下载原始文件</a></p>}</div>}</section><aside className="surface qm-metadata"><h2>资料信息</h2><dl><dt>所属项目</dt><dd><a href={`#${projectPath(project)}`}>{project}</a></dd><dt>业务类型</dt><dd>{kinds.get(data.kind) || data.kind}</dd><dt>制品版本</dt><dd>{data.revision}</dd><dt>正文字符数</dt><dd>{(data.content?.length ?? 0).toLocaleString()}</dd><dt>记录 ID</dt><dd className="qm-mono">{data.id}</dd><dt>正文 SHA-256</dt><dd className="qm-mono">{data.sha256}</dd><dt>项目与类型来源</dt><dd>外部输入 / 用户指定</dd></dl></aside></div><StructureDetail artifact={data} tenant={tenant} project={project} /></>;
}

function ArtifactPage({ tenant, project, id, kinds }: { tenant: string; project: string; id: string; kinds: Map<string, string> }) {
  const [data, setData] = useState<Artifact | null>(null), [content, setContent] = useState<string | null>(null), [contentReason, setContentReason] = useState(""), [contentLoading, setContentLoading] = useState(true), [error, setError] = useState(""), [raw, setRaw] = useState(false);
  useEffect(() => { let active = true; get<Artifact>(`/artifacts/${id}`, tenant, project).then(a => { if (active) setData({ ...a, content: undefined }); }).catch(e => { if (active) setError(errorText(e)); }); return () => { active = false; }; }, [id, tenant, project]);
  useEffect(() => { let active = true; setContentLoading(true); setContent(null); setContentReason(""); getArtifactContent(id, tenant, project).then(value => { if (!active) return; const nextContent = value.available ? value.content : null; setContent(nextContent); setContentReason(value.reason || "当前资料没有可用正文。"); setData(current => current ? { ...current, content: nextContent || undefined } : current); }).catch(e => { if (active) setContentReason(errorText(e)); }).finally(() => { if (active) setContentLoading(false); }); return () => { active = false; }; }, [id, tenant, project]);
  if (error) return <Alert>{error}</Alert>; if (!data) return <Loading />;
  return <div className="artifact-detail-page"><a className="qm-back" href={`#${projectPath(project)}`}><ArrowLeft size={15} />返回项目</a><header className="artifact-detail-header"><div><div className="eyebrow">ARTIFACT DETAIL</div><h1>{data.external_id}</h1><p>{data.locator}</p></div><a className="qm-button" href={url(`/artifacts/${id}/download`, tenant, project)}><Download size={17} />{data.original_file_id ? "下载原始文件" : "下载正文"}</a></header><div className="artifact-detail-grid"><main className="artifact-main"><section className="surface"><div className="panel-header"><h2>{raw ? "结构化记录" : "完整正文"}</h2><button onClick={() => setRaw(v => !v)}>{raw ? "查看正文" : "查看 JSON"}</button></div>{raw ? <pre className="qm-document" tabIndex={0}>{JSON.stringify(data, null, 2)}</pre> : contentLoading ? <div className="qm-empty" role="status">正在加载正文…</div> : content ? <pre className="qm-document" tabIndex={0}>{content}</pre> : <div className="qm-empty" role="status">{contentReason || "当前资料没有可显示的正文。"}{data.original_file_id && <p><a href={url(`/artifacts/${id}/download`, tenant, project)}>下载原始文件</a></p>}</div>}</section><StructureDetail artifact={data} tenant={tenant} project={project} /></main><aside className="surface qm-metadata"><h2>资料信息</h2><dl><dt>所属项目</dt><dd><a href={`#${projectPath(project)}`}>{project}</a></dd><dt>业务类型</dt><dd>{kinds.get(data.kind) || data.kind}</dd><dt>制品版本</dt><dd>{data.revision}</dd><dt>正文字符数</dt><dd>{(content?.length ?? 0).toLocaleString()}</dd><dt>记录 ID</dt><dd className="qm-mono">{data.id}</dd><dt>正文 SHA-256</dt><dd className="qm-mono">{data.sha256}</dd><dt>项目与类型来源</dt><dd>外部输入 / 用户指定</dd></dl></aside></div></div>;
}
...
function RunPage({ tenant, project, id, refresh, begin }: { tenant: string; project: string; id: string; refresh: number; begin: (r: Run | Run[]) => void }) {
  ...
  return <><a className="qm-back" href={batchPath(project, batchKey(run))}><ArrowLeft size={15} />返回所属批次</a><div className="qm-heading"><div><div className="eyebrow">TRACEABILITY RUN / {run.id.slice(0, 8)}</div><h1>TLR 检测结果 <span className={`mini-pill ${displayStatus}`}>{statusLabel[displayStatus]}</span></h1><p>{run.config.batch_label || (run.config.plan_id ? `批次 ${run.config.plan_id.slice(0,8)}` : "单次运行")} · 创建于 {date(run.created_at)}</p></div><div className="inline-controls"><button onClick={() => setTick(v => v + 1)}><RefreshCw size={16} />刷新结果</button>{run.status === "pending" && <button className="primary-button" onClick={() => begin(run)}>执行此运行</button>}{canResume && <button className="primary-button" onClick={() => begin(run)}>从断点继续此运行</button>}</div></div>
    ...
    <details className="surface qm-run-info"><summary>查看运行配置与复现信息</summary><pre>{JSON.stringify({ config: run.config, manifest: run.manifest, counts: run.counts }, null, 2)}</pre></details>
  </>;
}
