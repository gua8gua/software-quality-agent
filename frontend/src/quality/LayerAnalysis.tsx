...
export function LayerAnalysis({tenant,project,dataset,artifacts,kinds,close,created}: {tenant:string;project:Project;dataset:Dataset;artifacts:Artifact[];kinds:Map<string,string>;close:()=>void;created:(runs:Run[])=>void}) {
  ...
  return <Modal title="选择 TLR 层间比较" wide close={()=>!busy&&close()}><form onSubmit={submit}>
    <p><strong>{project.name}</strong> / {dataset.version}</p><p className="muted-text">行是源层，列是目标层；可多选方向。默认连接有正文的相邻非空层，空层跳过；层次顺序是本项目的工作流约定，不代表金标准。</p>
    {error&&<Alert>{error}</Alert>}{!inventory?<Loading/>:<>
      <div className="inline-controls"><button type="button" onClick={()=>setPairs(inventory.default_pairs.map(p=>p.join("|")))}>恢复相邻层默认</button><button type="button" onClick={()=>setPairs([])}>清空选择</button><button type="button" onClick={()=>setManual(true)}>手工选择源与目标制品</button></div>
      <div className="table-wrap qm-layer-selection"><table className="data-table"><thead><tr><th>源 ↓ / 目标 →</th>{inventory.layers.map(l=><th key={l.id}>{l.label}<small>{l.count} 份正文</small></th>)}</tr></thead><tbody>{inventory.layers.map(s=><tr key={s.id}><th>{s.label}<small>{s.count} 份正文</small></th>{inventory.layers.map(t=><td key={t.id}><input type="checkbox" aria-label={s.label+" → "+t.label} disabled={busy||s.id===t.id||!s.count||!t.count} checked={pairs.includes(s.id+"|"+t.id)} onChange={()=>toggle(s.id+"|"+t.id)}/></td>)}</tr>)}</tbody></table></div>
      <p className="muted-text">{inventory.excluded_ids.length} 个包或仅有引用的节点已排除。每个单元格产生独立运行，保留分割配置、候选与最终链接；当前页面依次执行，关闭页面会中断后续排队，已创建的待执行运行仍可从历史进入继续。</p>
      ...
      <footer><span>已选 {pairs.length} 个层间任务</span><button type="button" onClick={close} disabled={busy}>取消</button><button className="primary-button" disabled={busy||!pairs.length}>{busy?"创建任务…":"创建并依次检测"}</button></footer>
    </>}
  </form></Modal>;
}
...
export function LayerRunMatrix({runs,project,datasetId,resume,fixedPlanId,currentRunId}:{runs:Run[];project:string;datasetId:string;resume:(runs:Run[])=>void;fixedPlanId?:string;currentRunId?:string}){
  ...
  return <section className="qm-inset"><div className="inline-controls"><h3>{label} · {rows.length} 个任务</h3>{!fixedPlanId&&<select aria-label="选择比较批次" value={current} onChange={e=>setSelected(e.target.value)}>{ids.map(id=>{const row=available.find(run=>run.config.plan_id===id);return <option key={id} value={id}>{row?.config.batch_label||`批次 ${id.slice(0,8)}`}</option>})}</select>}<button className="primary-button" disabled={!resumable.length} onClick={()=>resume(resumable)}>恢复整个批次（{resumable.length}）</button></div><div className="table-wrap"><table className="data-table"><thead><tr><th>源 ↓ / 目标 →</th>{layers.map(l=><th key={l}>{names[l]}</th>)}</tr></thead><tbody>{layers.map(s=><tr key={s}><th>{names[s]}</th>{layers.map(t=>{const r=rows.find(r=>r.config.layer_pair?.[0]===s&&r.config.layer_pair?.[1]===t);if(!r)return <td key={t}>—</td>;const display=runDisplayStatus(r);return <td key={t} className={r.id===currentRunId?"current":""}><a href={"#/projects/"+encodeURIComponent(project)+"/runs/"+r.id}><span className={"mini-pill "+display}>{statusLabel[display]}</span><small>{r.status==="completed"?(r.counts.links||0)+" 条链接":r.status==="pending"?"等待执行":r.status==="running"?"正在执行":"可从断点恢复"}</small></a></td>})}</tr>)}</tbody></table></div></section>
}
